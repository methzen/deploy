...
In this guide, we'll deploy a Node.js application on a DigitalOcean server in the cloud with SSL/HTTPS encryption and a custom domain.
...
Table of Contents
    ...
    Create & Configure A New DigitalOcean Server
...
Before we can do anything, we need to create and configure a VPS (Virtual Private Server) in the cloud to host your Node.js application. There are a lot of companies that provide this service, but we'll use DigitalOcean.
...
For a FREE $200 CREDIT FOR 60 DAYS, use this link to sign up: https://m.do.co/c/ce20017d8588.
...
DigitalOcean Create New Droplet Page - Select Plan
...
DigitalOcean Create New Droplet Page - Choose Datacenter Region
...
Before proceeding to the next step, make sure you have purchased a domain name from a service like GoDaddy, namecheap (my personal favorite), HostGator, name.com, or another registrar.
Configure DNS
...
DigitalOcean Create New Domain
...
DigitalOcean Add DNS Record Example Dot Com

For the second one, enter www in the HOSTNAME field and select the same server:
DigitalOcean Add DNS Record WWW Dot Example Dot Com
...
To set up your domain on other registrars, there are more guides here.

You should now have a domain pointing at your newly created DigitalOcean server.
Step 3 - Install & Configure Nginx

Now that your domain is pointing to your server, it's time to install Nginx and configure your server to host web content.
